...
class DiscreteCalculator(ttk.Frame):
    def __init__(self, parent):
        ...
        self.a_f = DoubleVar(value = 1058.0)         # final (target) altitude
        ...
        self.delta_longitude_degrees = DoubleVar()   # longitude difference to target during burn
        self.delta_longitude_radians = 0             # radians version for calculations
        self.burn_distance = DoubleVar(value = 1000) # distance to target during burn 
        ...
        ttk.Label(self.metrics_frame, text="Target altitude").grid(column=0, row=1)
        self.a_f_Entry = ttk.Entry(self.metrics_frame, textvariable = self.a_f)
        self.a_f_Entry.grid(column = 1, row = 1)
        self.a_f.trace_add("write", self.altitude_changed)
        ttk.Label(self.metrics_frame, text="km").grid(column = 2, row = 1)

        ttk.Label(self.metrics_frame, text = "Burn delta longitude").grid(column = 0, row = 2)
        self.delta_longitude_degrees_label = ttk.Label(self.metrics_frame, textvariable = self.delta_longitude_degrees)
        self.delta_longitude_degrees_label.grid(column = 1, row = 2)
        ttk.Label(self.metrics_frame, text="⁰").grid(column = 2, row = 2)

        ttk.Label(self.metrics_frame, text = "Distance to target").grid(column = 0, row = 3)
        self.burn_distance_label = ttk.Label(self.metrics_frame, textvariable = self.burn_distance)
        self.burn_distance_label.grid(column = 1, row = 3)
        ttk.Label(self.metrics_frame, text="km").grid(column = 2, row = 3)
        ...
        ttk.Label(self.metrics_frame, text = "Transfer duration").grid(column = 0, row = 8)
        ...
        self.final_orbit = Circle((0,0), transform=self.ax.transData._b, edgecolor = "skyblue", facecolor = '#00000000')
        self.transfer_orbit = Line2D([0,1],[0,1], color = "limegreen")
        self.ship, = self.ax.plot(0, 0, color = "white", marker = "^", markersize = 15)
        self.station, = self.ax.plot(0, 0, color = "skyblue", marker = "D", markersize = 12)
        self.rendezvous_point, = self.ax.plot(0, 0, color = "orange", marker = "X", markersize = 12)
    ...
    def circular_orbit_progress_angle(self, r, t):
        return math.tau*(t/math.sqrt((4*(math.pi**2)/(self.scaled_G*self.planet_M))*r**3))
    ...
    def calc_trajectory(self):
        # ~ global self.encounter_exists, self.initial_state
        ...
        while(
            ...
            and (abs(result[-1][0,0] - self.r_f) > 10)  # Stop when within 10km of target altitude
        ...
        if abs(result[-1][0,0] - self.r_f) < 10:
    ...
    def update_parameters(self):
        if self.encounter_exists:
            ...
            self.delta_longitude_radians = self.trajectory[1,-1]-self.circular_orbit_progress_angle(self.r_f, transfer_duration_secs)
            self.delta_longitude_degrees.set(round(math.degrees(self.delta_longitude_radians), 1))
            
        else:
            self.delta_longitude_degrees.set("N/A")
        ...
        self.burn_distance.set(round((np.sqrt(self.r_i**2+self.r_f**2 - 2*self.r_i*self.r_f*math.cos(self.delta_longitude_radians))), 1))
        try:
    ...
    def update_plot(self):
        ...
        self.final_orbit.set(radius = self.r_f)
        self.transfer_orbit.set_data(self.trajectory[1],self.trajectory[0])
        self.ship.set_data([0], [self.r_i])
        if self.encounter_exists:
            self.rendezvous_point.set_data([self.trajectory[1,-1]], [self.r_f])    # set orange cross at the end of the trajectory
            self.rendezvous_point.set_visible(True)
            # set station location
            self.station.set_data([self.delta_longitude_radians], [self.r_f])   # put station where it would be at the time of the burn
            self.station.set_visible(True)
