...
class ReentryCalculator(ttk.Frame):
    ...
    def mark_invalid(self, entry_widget):
        entry_widget.config(
    ...
    def reset_border(self, entry_widget):
        entry_widget.config(
    ...
    def altitude_changed(self, *args):
        ...
        try:
            ...
            self.mark_invalid(self.a_i_Entry)
            return
        try:
            self.r_f = self.a_f.get() + self.planet_R
        except TclError:
            # ~ print("You entered something strange")
            self.mark_invalid(self.a_f_Entry)
            return
        for widget in [self.a_i_Entry, self.a_f_Entry]:
            reset_border(widget)
